...
function getObjectName(objectIndex) {
    ...
    if (myLang == "FR")
        ...
        r = objList[objectIndex].ObjectEn
    return r;
}
...
function getObjectBySize(aSizeInMeter) {
    var tolerance = 0.1;
...
function rebuildTable() {
    ...
    if (ob1Name != "" & ob2Name != "") {
        ...
        var htmlRowTemplate = "<tr>"
            ...
            + "<td data-label='" + labelList[8] + "'>%sc</td>"
            + "<td data-label='" + labelList[10] + "'>%on</td>"
            + "</tr>";
        oldClass = 'scMicro';
        ...
        for (i = 0; i < objList.length; i++) {
            ...
            curMagnitudeObjName=getObjectBySize(scaledSize);        
       ...
       //        console.log(curName,curSize ,"m ",   curSizeInUnit ,  curUnitSymbol," SCALED:",scaledSize , "m ",scaledSizeInUnit, scaledUnitSymbol ,curMagnitudeObjName);
